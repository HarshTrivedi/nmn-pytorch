...
class VQAEval:
    ...
    def evaluate(self, quesIds=None):
        ...
        print "Done computing accuracy - 1"
        ...
        for quesId in quesIds:
            ...
            for gtAnsDatum in gts[quesId]['answers']:
                ...
                max_acc = -1
                resAnsSet = res[quesId]['answers']
                for resAnsInSet in resAnsSet:

                    # resAns = res[quesId]['answers'][0]
                    resAnsInSet = resAnsInSet.replace('\n', ' ')
                    resAnsInSet = resAnsInSet.replace('\t', ' ')
                    resAnsInSet = resAnsInSet.strip()
                    resAnsInSet = self.processPunctuation(resAnsInSet)
                    resAnsInSet = self.processDigitArticle(resAnsInSet)

                    matchingAns = [
                        item for item in otherGTAns
                        if item['answer'] == resAnsInSet
                    ]
                    acc = min(1, float(len(matchingAns)) / 3)
                    if acc > max_acc:
                        max_acc = acc
                gtAcc.append(max_acc)
            # quesType = gts[quesId]['question_type']
            # ansType = gts[quesId]['answer_type']
            avgGTAcc = float(sum(gtAcc)) / len(gtAcc)
            accQA.append(avgGTAcc)
        ...
        self.setAccuracyV2(accQA)
        print "Done computing accuracy - 2"

    def processPunctuation(self, inText):
        outText = inText
        for p in self.punct:
            if (p + ' ' in inText or ' ' + p in inText) or (re.search(
                    self.commaStrip, inText) != None):
    ...
    def setAccuracy(self, accQA, accQuesType, accAnsType):
        ...
        self.accuracy['perQuestionType'] = {
            quesType: round(100 * float(sum(accQuesType[quesType])) /
                            len(accQuesType[quesType]), self.n)
    ...
    def setAccuracyV2(self, accQA):
        self.accuracy['overall-top-3'] = round(
