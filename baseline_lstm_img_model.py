...
class LSTMModel(torch.nn.Module):
    ...
    def __init__(self, question_word_embeddings_np, embedding_dim, hidden_dim,
                 vocab_size):
...
class VGABaseModel(torch.nn.Module):
    ...
    def __init__(self, D_img, D_txt, D_map, D_hidden,
        ...
        self.lstmModel = LSTMModel(question_word_embeddings_np, D_txt, D_hidden,
                                   question_vocab_size)

    def forward(self, image_feat, token_sequence_tensor):

        image_feat_mapped = self.squeeze_image_to_map_d(
            image_feat.sum(2).sum(2))
        lstm_encoded = self.lstmModel(token_sequence_tensor)

        lstm_encoded_mapped = self.squeeze_wordvec_to_map_d(lstm_encoded)
        elementwise_multiplication = torch.mul(lstm_encoded_mapped,
                                               image_feat_mapped)
