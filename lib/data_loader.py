...
class VqaDataset(Dataset):
    ...
    def __init__(self, vocab_answer_file, vocab_module_file, vocab_question_file, wordvec_file, annotation_file, question_file, module_layout_file, image_feature_file, transform=None):
        ...
        with open(annotation_file) as f:
            ...
            qid2annotation_dict = {annotation['question_id']: annotation for annotation in annotations}
    ...
    def __getitem__(self, idx):
        ...
        valid_answers = [ answer["answer"] for answer in annotation['answers'] if answer["answer"] in self.valid_answer_set]
        ...
        image_feature_tensor = np.load( self.image_feature_file % image_id )['arr_0']
        question_token_ids = np.array([ self.question_vocab_dict.word2idx(token) for token in question_tokens])

        datum = {'image_feature_tensor': image_feature_tensor, 
                 'layout': layout,
    ...
    def show_batch(self, sample_batched):
        ...
        answer_vector = sample_batched['answer_vector_tensor'][0].numpy().reshape([-1])
        qid           = int(sample_batched['question_id'][0].numpy())

        question = [ q for q in self.questions_dict if q['question_id'] == qid ][0] 
        ...
        actual_answers_ids  = [ self.answer_dict.word2idx(a['answer']) for a in annotation['answers']]
        constructed_answer_ids = list(np.nonzero(answer_vector))[0]
        constructed_answers_text = [ self.answer_dict.idx2word(answer_id) for answer_id in constructed_answer_ids]

        question_text = question['question']
        image_id = question['image_id']
        question_text_constructed = [ self.question_vocab_dict.idx2word(token_id) for token_id in qtoken_ids]
    ...
    @staticmethod
    def get_image_feature_mean_and_stds( question_file, image_feature_file, saved_mean_stds_file):
        ...
        if os.path.exists(saved_mean_stds_file):
            ...
            vals = np.zeros( [image_feat_D, images_count*image_feat_H*image_feat_W ] )
            
            filled_upto = 0
            for done, image_id in enumerate(image_ids):
                if done % 1000 == 0: print done
                image_feature_tensor = np.load( open(image_feature_file % image_id) )['arr_0']
                for channel in range(image_feat_D):
                    vector = image_feature_tensor[channel,:,:].ravel() #14X14
                    vals[channel, filled_upto: (filled_upto+(image_feat_H*image_feat_W))] = vector
...
class ToTensor(object):
    ...
    def __call__(self, datum):
        ...
        torch_datum = { 'image_feature_tensor': torch.from_numpy(image_feature_tensor),  
                        'layout': layout,
...
class Normalize(object):
    ...
    def __call__(self, torch_datum):
        ...
        tensor_normalize = transforms.Normalize( mean= self.means, std= self.stdevs )
        normalized_image_feature_tensor = tensor_normalize(image_feature_tensor)

        normalized_torch_datum = {  'image_feature_tensor': normalized_image_feature_tensor, 
                                    'layout': str(layout), 
                                    'answer_index': answer_index, 
                                    'question_token_ids': question_token_ids,
...
def get_dataset(image_set, use_mean_stds, saved_mean_stds_file = None):
    ...
    vocab_answer_file  = os.path.join( root_dir, 'preprocessed_data/vqa_answers.txt' )
    vocab_module_file  = os.path.join( root_dir, 'preprocessed_data/vocabulary_modules.txt' )
    vocab_question_file= os.path.join( root_dir, 'preprocessed_data/vocabulary_vqa.txt' )
    wordvec_file       = os.path.join( root_dir, 'preprocessed_data/vocabulary_vqa_glove.npy' )
    ...
    if use_mean_stds: # In case of Test/Val (stats would be given)
        ...
        means, stdevs = VqaDataset.get_image_feature_mean_and_stds( question_file % image_set,
    ...
    vqa_dataset = VqaDataset( 
                            ...
                            image_feature_file % (image_set, image_set),
                            transform = transforms.Compose( [ ToTensor(), Normalize(means, stdevs) ] )
                            )
