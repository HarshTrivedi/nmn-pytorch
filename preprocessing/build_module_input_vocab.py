...
module_layouts_dir = os.path.join(root_dir, 'preprocessed_data/layouts/module_layouts')
module_layout_file = os.path.join( module_layouts_dir, 
                    'questions_module_layouts_{}.txt'.format(image_set) )
...
with open(module_layout_file ) as f:
    for line in f.readlines():
        if line.strip() != "":
            ...
            for token in set(flatten(layouts)) - set(["Find", "Describe", "Transform", "And", "Or"]):
