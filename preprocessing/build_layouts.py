...
for set_name in set_names:
    ...
    question_file = os.path.join(
        root_dir,
        'raw_data/Questions/v2_OpenEnded_mscoco_{}_questions.json'.format(
            set_name))

    question_ids_file = os.path.join(question_ids_dir,
                                     'questions_{}.txt'.format(set_name))

    raw_question_file = os.path.join(
        layout_data_dir, 'raw_questions/questions_{}.txt'.format(set_name))
...
for set_name in set_names:
    print "{} ...".format(set_name)
    raw_question_file = raw_question_file = os.path.join(
        layout_data_dir, 'raw_questions/questions_{}.txt'.format(set_name))

    question_parse_tree_file = os.path.join(
        question_parse_trees_dir,
...
for set_name in set_names:
    ...
    primary_layout_file = os.path.join(
        primary_layouts_dir,
...
for set_name in set_names:
    ...
    module_layout_file = os.path.join(
        module_layouts_dir, 'questions_module_layouts_{}.txt'.format(set_name))
    ...
    with open(primary_layout_file) as f:
        for line in f.readlines():
            ...
            if parse_group != "":
                ...
                def flatten(lol):
                    return sum([flatten(l) for l in lol], []) if isinstance(
                        lol, (tuple, list)) else [lol]

                layouts = [
                    layout for layout in layouts
                    if (flatten(layout).count('Describe') == 1)
                ]
                layouts = [
                    layout for layout in layouts
                    if (len(layout) == 2 or layout[0] == 'Find')
                ]

                picked_layout = layouts[
                    0]  # Fix this Layout Picker to something better!!!
    ...
    with open(module_layout_file, 'w') as f:
        lines = [
            "\t".join([question_id, layouts])
            for question_id, layouts in zip(question_ids,
                                            map(str, question_wise_layouts))
        ]
        f.write('\n'.join(lines))
