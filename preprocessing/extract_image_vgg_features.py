...
IMAGE_ROOT = os.path.join(root_dir, 'raw_data/Images/{}/'.format(image_set))
IMAGE_CONV_DEST = os.path.join(
    root_dir, 'image_features/preprocessed_data/{}/'.format(image_set))
...
net = caffe.Net(VGG_DEF, VGG_MODEL, caffe.TEST)
...
for size, names in image_names_by_size.items():
    ...
    while i < n_images:
        ...
        names_here = [
            os.path.join(IMAGE_ROOT, names[i + j]) for j in range(real_count)
        ]
        ...
        net.blobs["data"].reshape(real_count, 3, images[0].shape[0],
                                  images[0].shape[1])
        ...
        transformer.set_mean("data",
                             np.load(IMAGENET_MEAN_FILE).mean(1).mean(1))
        transformer.set_raw_scale("data", 255)
        transformer.set_channel_swap("data", (2, 1, 0))

        proc_images = [
            transformer.preprocess("data", image) for image in images
        ]
        ...
        for j in range(real_count):
            ...
            np.savez(IMAGE_CONV_DEST + "/" + names[i + j],
                     embeddings[j, :, :, :])
